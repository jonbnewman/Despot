...
class Despot {
	...
	public static function init() {
		...
		set_exception_handler( function($exception) { Despot::exceptionHandler($exception); } );
  ...
  public static function getPath( $pathType, $base = false ) {
	 ...
	 *	@example Despot::route('/notWhatYouThink/', 'misc/foobar') makes http://foo.com/notWhatYouThink/ route through controller misc/foobar
	 *
	...
	public static function castController( $controller ) {
		...
		if( is_int($controller['controller']) ) {
			$controller = array('controller' => Despot::getPath('controllers', true).'/'.self::$routes[$controller['controller']], 'arguments' => $controller['arguments']);
		}
	...
	*	@return string File name after sanitation
	 ...
	 *
	 *	@example Despot::loadModel('test.model.php'); will create an object called $test (unless specified using $objectName) using the class 'test' which is inside that model file.
	 ...
	 *
	 *	@example Despot::loadLibrary('test.library'); // (require_once's library file from %APPDIR%/library/test.library.php)
	...
	/**
	 *	Crawls the user-defined routes (Despot::setRoute()) looking for the appropriate controller for the given $uri
	 *
	 *	@param string $uri URI we wish to attempt to 'crawl'
	 *	@param int    $pieces Specify the number of 'pieces' we want from the supplied $uri
